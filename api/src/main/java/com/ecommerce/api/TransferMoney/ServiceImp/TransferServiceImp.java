...
public class TransferServiceImp implements TransferService {
    ...
    public String trs(TransfertDto transfertDto, long client_id, long bene_id, BeneficiaryDto bene) {
        ...
        if(transfertDto.getTypeOftransfer() == Type_transfer.ACCOUNT_DEBIT) {
            ...
            if (userOptional.isPresent()) {
                ...
                //transfertDto.setAmount_transfer(transfertDto.getAmount_entred().divide(new BigDecimal(100)));
                ...
                if (transfertDto.isNotification()) {
                    emailService.SendEmailAlert(EmailDetails.builder()
                            .subject("Your accunt is credited")
                            .recipient(beneficiary.getUsername())
                            .message(beneficiary.getFirstName() + " " + "you received money from " + " " + user.getUsername() + " \n" + "your transfer reference : " + "  " + transfertDto.getGenerateRef() + " \n" + " " + "don't share this with anyone!"
                                    + "\n" + "  " + "le montant total de l'opération est: " + transfertDto.getAmount_total()
                                    + " " + "le montant de transfert est: " + transfertDto.getAmount_transfer())
                            .build());
            ...
            return "an error occurred this method for a transfer by account debit!"+ " "+ transfertDto.getTypeOftransfer();
        }
    ...
    public void processTransaction(User user) {
        ...
        if (prospects != null) {
            ...
            // If Prospects entity doesn't exist, create a new one within the same transaction
    ...
    public String checkAmountOfTransfert(TransfertDto transfertDto, User user, BigDecimal checkAmount) {
        ...
        if(transfertDto.getAmount_entred().compareTo(transferUtils.getMinTotal())<0){
            return "you must make a transfer with a minimum amount of" +" "+ transferUtils.MinTotal +" dh !" +" actually your transfer amount is : " + transfertDto.getAmount_transfer();
        }
    ...
    public void ExpenseManagement(TransfertDto transfertDto) {
        ...
        if(transfertDto.isNotification()){
            ...
            if(transfertDto.getFees().equals(TypeOfFees.donor)){
                transfertDto.setAmount_total(transfertDto.getAmount_entred().add(transferUtils.getFraiDuTransfert()).add(transferUtils.getNotificationCosts()));
            ...
            if(transfertDto.getFees().equals(TypeOfFees.beneficiary)){
                ...
                transfertDto.setAmount_transfer(transfertDto.getAmount_entred().subtract(transferUtils.fraiDuTransfert));
            }
            if(transfertDto.getFees().equals(TypeOfFees.shared)){
                transfertDto.setAmount_total(transfertDto.getAmount_entred().add((transferUtils.fraiDuTransfert).divide(new BigDecimal(2))).add(transferUtils.getNotificationCosts()));
                transfertDto.setAmount_transfer(transfertDto.getAmount_entred().subtract((transferUtils.fraiDuTransfert).divide(new BigDecimal(2))));
            }
        }
            ...
            if(transfertDto.getFees().equals(TypeOfFees.shared)){
                transfertDto.setAmount_total(transfertDto.getAmount_entred().add((transferUtils.fraiDuTransfert).divide(new BigDecimal(2))));
                transfertDto.setAmount_transfer(transfertDto.getAmount_entred().subtract((transferUtils.fraiDuTransfert).divide(new BigDecimal(2))));
            }
    ...
    public void DebitCreditAccount(TransfertDto transfertDto, User user, Beneficiary beneficiary) {
        if (user != null && user.getAccount_amount() != null && beneficiary != null && beneficiary.getAccount_amount() != null) {
